...
**0.1.0** (alpha) — first tagged preview, 2026-04-26. All four storage backends (fs-json, fs-markdown, surreal, postgres) are full-CRUD; auth surface is wired end-to-end (Argon2id + JWT iat-invalidation + refresh rotation + RFC 6238 TOTP + RBAC + per-IP rate limit + CSRF); GraphQL + REST + SSE + WebSocket subs are co-equal; admin UI runs on Leptos SSR + CSR; the wasmtime+component-model plugin host loads real WASM components with capability gates. Performance budgets are CI-enforced (per DESIGN.md §10). The 0.1.0 tag marks the **feature-preview** milestone — stabilization, hardening, and SemVer guarantees land at **1.0**. See [`CHANGELOG.md`](CHANGELOG.md) for the full version arc and [`DESIGN.md §13`](DESIGN.md#13-roadmap) for the roadmap to 1.0 (edge runtime is v2 — see ADR-0005).
