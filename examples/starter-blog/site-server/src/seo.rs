...
/// SEO sidecar file produced by the `plugin-seo` WASM plugin on
/// `content.published`. Layout written by the plugin:
///
/// ```json
/// { "open_graph": { "og:title": "...", ... }, "json_ld": { "@type": "...", ... } }
/// ```
...
impl SeoLoader {
    ...
    /// Load the sidecar for `<root>/<type_slug>/<slug>.json`. Returns
    /// `None` (not Err) for missing files so callers can degrade gracefully
    /// when the plugin hasn't been installed.
...
/// Render OG meta tags + JSON-LD `<script>` for a `SeoMeta`. Output is
/// HTML-safe; the plugin's JSON values are escaped before injection.
pub fn render_head(meta: &SeoMeta) -> String {
    ...
    if !meta.json_ld.is_null() {
        // JSON-LD lives inside <script type="application/ld+json"> — escape
        // `</script>` defensively to prevent context break-out.
        let ld = serde_json::to_string(&meta.json_ld).unwrap_or_else(|_| "{}".into());
        let safe = ld.replace("</script", "<\\/script");
        out.push_str(&format!(
            r#"<script type="application/ld+json">{safe}</script>"#
        ));
    }
    out
}
...
mod tests {
    ...
    fn escapes_meta_attributes() {
        ...
        let html = render_head(&meta);
        assert!(html.contains(r#"content="q&quot;x""#));
    }
}
