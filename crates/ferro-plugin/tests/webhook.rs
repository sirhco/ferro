...
async fn webhook_delivers_signed_event() {
    ...
    registry
        .dispatch(HookEvent::ContentCreated { content: permitted, type_slug: Some("post".into()) })
        .await;
    registry
    ...
    assert_eq!(headers.get("x-ferro-event").unwrap().to_str().unwrap(), "content.created");
...
async fn webhook_without_filter_delivers_everything() {
    ...
    let app = Router::new()
        .route(
            "/hook",
            post(|State(c): State<Captured>, _h: HeaderMap, body: Bytes| async move {
